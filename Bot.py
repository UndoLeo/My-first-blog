...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text('Выберите тип:', reply_markup=reply_markup)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data in ['beacon', 'pier']:
        ...
        await query.edit_message_text(text="Выберите цвет:", reply_markup=reply_markup)
    else:
        ...
        if link:
            await query.edit_message_text(text=f"Ваша ссылка: {link}")
        else:
            await query.edit_message_text(text="Ошибка: цвет не найден")
